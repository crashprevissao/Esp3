...
static void IRAM_ATTR promiscuousCallback(void* buf, wifi_promiscuous_pkt_type_t type) {
    ...
    // 802.11 MAC header: addr2 (src/transmitter) at byte 10
    ...
    // addr1 (dst/receiver) at byte 4 — catches Flock STAs receiving probe responses
    const uint8_t* addr1 = payload + 4;
    if (matchOui(addr1)) {
        enqueueEvent(addr1, pkt->rx_ctrl.rssi, pkt->rx_ctrl.channel);
    }
}
